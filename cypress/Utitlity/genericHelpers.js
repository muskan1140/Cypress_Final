...
Cypress.Commands.add("selectActivity", (activityNumber) => {
    cy.get("[data-testid='activity-pagination'] li").eq(activityNumber).click()
})
...
class GenericActions {
    ...
    clearText(element) {
        element.clear().then(function () {
            cy.log(colors.green('             Clearing of field !!!'));
        }, function (err) {
            cy.log(colors.red('--->Error: Clearing did not perform due to: ' + err));
            //throw err;
        });
    }
    ...
    typeText(element, data) {
        ...
        element.clear().type(data).then(function () {
            cy.log(colors.green('             Typing of the field with value: ' + data));
        }, function (err) {
    ...
    drag(sourceElement, targetElement) {
        ...
        cy.get(sourceElement).drag(targetElement).then(function () {
            cy.log(colors.green('Element is dragged from ' + sourceElement + ' to ' + targetElement));
        }, function (err) {
            cy.log(colors.red('--->Error: Unable to drag Element due to' + err));
            //throw err;
        });
    }
    ...
    click(element) {
        ...
        element.click().then(function () {
            cy.log(colors.green("             The element got clicked."));
        }, function (err) {
            cy.log(colors.red("--->Error: The element couldn't get clicked due to: " + err));
            //throw err;
        });
        ...
        try {
            return element.invoke('text').then(function (text) {
                return new Promise(function (resolve, reject) {
                    cy.log(colors.green("             The text of element is captured which is: " + text));
                    resolve(text);
                }
                )
            })
            ...
            cy.log(colors.red("--->Error: The text of the element couldn't be captured due to: " + err));
        }
    ...
    shouldHaveText(element, text) {
        element.should('have.text', text).then(function (text) {
            cy.log(colors.green("             The element is have: " + text));

        }, function (err) {
            cy.log(colors.red("--->Error: The element dosn't have text due to: " + err));
            //throw err;
        });
    ...
    shouldHaveValue(element, value) {
        element.should('have.value', value).then(function (text) {
            cy.log(colors.green("             The element have value: " + value));

        }, function (err) {
            cy.log(colors.red("--->Error: The element dosn't have value due to: " + err));
            //throw err;
        });
    ...
    shouldContainText(element, text) {
        element.should('contain', text).then(function (text) {
            cy.log(colors.green("             The element contain: " + text));

        }, function (err) {
            cy.log(colors.red("--->Error: The element dosn't contain text due to: " + err));
            //throw err;
        });
    ...
    shouldHaveCSS(element, cssname, value) {
        element.should('have.css', cssname, value).then(function (text) {
            cy.log(colors.green("             The element have css value: " + value));

        }, function (err) {
            cy.log(colors.red("--->Error: The element dosn't have css expected css value due to: " + err));
            //throw err;
        });
    ...
    getAttribute(element, attribute) {
        element.invoke('attr', attribute).then(function (text) {
            cy.log(colors.green("             The attribute of element is captured which is: " + text));
            return text;
        }, function (err) {
            cy.log(colors.red("--->Error: The attribute of the element couldn't be captured due to: " + err));
            //throw err;
        });
    ...
    selectDropDownUsingText(element, text) {
        ...
        element.select(text).then(function () {
            cy.log(colors.green("--->Success: The " + text + " in dropdown got clicked."));
        }, function (err) {
            cy.log(colors.red("--->Error: The " + text + " in dropdown couldn't get clicked due to: " + err));
        });
    ...
    genarateemailaddress() {
        ...
        email = 'wseautomation' + string + +randomnumber + '@wseautomationemail.com'
        cy.log(email)
        return email;
    }
